...
![png](/assets/img/naive-bees2_files/naive-bees2_3_1.png)
...
```
    ...
    <matplotlib.image.AxesImage at 0x7fb59728b5f8>
...
```
    ...
    <ipython-input-16-c637427318ba> in <module>()
          2                                                     labels.genus.values,
          3                                                     test_size=.3,
    ...
    /usr/local/lib/python3.5/dist-packages/sklearn/utils/validation.py in _num_samples(x)
        ...
        118             raise TypeError("Singleton array %r cannot be considered"
    --> 119                             " a valid collection." % x)
        120         return x.shape[0]
        121     else:
    

    TypeError: Singleton array array(PCA(copy=True, iterated_power='auto', n_components=500, random_state=None,
